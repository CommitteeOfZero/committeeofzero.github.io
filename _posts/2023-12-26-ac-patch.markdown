...
The Partial patch adds subtitles for songs and voice-only lines, but in the interest of immersion in regards to the EN dub, does not fix series-wide inconsistencies and grammar mistakes. This is because these changes would cause the written lines to occasionally not match the spoken ones in the dub.
